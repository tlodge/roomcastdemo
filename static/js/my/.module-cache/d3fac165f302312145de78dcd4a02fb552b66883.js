define(['jquery','react', 'mixins', 'modal'], function($, React, mixins, modal){
		...
		getButtonsFromServer: function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					this.setState({buttons:data, buttonwidth:Math.ceil($(window).width()/data.length), buttonheight:Math.ceil($(window).height()/this.maxbuttons)});
	...
	var ButtonList = React.createClass({displayName: "ButtonList",
		...
		handleExternalHide: function() {
			...
			//prevent any clicks of buttons immediately after!
			window.setTimeout(function(){this.allowedtoshow = true}.bind(this), 1000);
		},
