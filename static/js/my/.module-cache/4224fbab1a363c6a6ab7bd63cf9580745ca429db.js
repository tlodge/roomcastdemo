	...
	var MainPanel = React.createClass({displayName: "MainPanel",
			...
			$.ajax({
				...
				success: function(data){
					...
					data.forEach(function(item, i){
						this.maxbuttons = Math.max(maxbuttons, item.buttons.length);
  					});		
		...
		componentDidMount: function(){
			...
			$(window).resize(function(){
				this.setState({buttonwidth:Math.floor($(window).width()/data.length), buttonheight:Math.floor($(window).height()/this.maxbuttons)});
			});
		},
