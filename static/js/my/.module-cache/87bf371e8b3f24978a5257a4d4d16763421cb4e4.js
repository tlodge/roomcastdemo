	...
	var ButtonList = React.createClass({displayName: "ButtonList",
		...
		render: function(){
			...
			var columns = this.props.buttons.map(function(item, i){
				...
				var buttoncount = buttoncount + item.buttons.length;
				
				return (
					 React.createElement(ButtonColumn, {buttoncount: buttoncount, colours: this.props.colours, clickhandler: this.handleModalClick, column: i, left: left, buttonwidth: this.props.buttonwidth, buttonheight: this.props.buttonheight, category: item.category, buttons: item.buttons})
				);
			}.bind(this));
