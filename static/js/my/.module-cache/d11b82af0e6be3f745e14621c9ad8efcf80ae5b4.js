		...
		getButtonsFromServer: function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					this.setState({buttons:data, width:$(window).width(), height:$(window).height()});
		...
		render: function(){
			...
			return (
				React.createElement("div", {className: "buttonpanel"}, 
					React.createElement(ButtonList, {buttons: this.state.buttons, width: this.state.width, height: this.state.height})
				)
			)
		}
	});
