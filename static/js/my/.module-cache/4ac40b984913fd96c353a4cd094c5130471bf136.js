		...
		getButtonsFromServer: function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					this.setState({buttons:data, buttonwidth:$(window).width()/data.length, buttonheight:$(window).height()/this.maxbuttons});
	...
	var ButtonList = React.createClass({displayName: "ButtonList",
		...
		render: function(){
			...
			var columns = this.props.buttons.map(function(item, i){
				...
				return (
					 React.createElement(ButtonColumn, {column: i, left: this.left, buttonwidth: self.props.buttonwidth, buttonheight: self.props.buttonheight, category: item.category, buttons: item.buttons})
				);
			});
