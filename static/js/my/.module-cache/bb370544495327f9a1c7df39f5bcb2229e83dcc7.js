define(['jquery','react', 'mixins', 'modal'], function($, React, mixins, modal){
	...
	var MainPanel = React.createClass({displayName: "MainPanel",
		...
		getButtonsFromServer: function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					this.setState({buttons:data, buttonwidth:Math.ceil($(window).width()/data.length), windowheight:Math.ceil($(window).height())});
		...
		getInitialState: function(){
			return {buttons:[], buttonwidth:0, windowheight:0};
		},
		
		handleResize: function(){
			this.setState({buttonwidth:Math.ceil($(window).width()/this.state.buttons.length), windowheight:Math.ceil($(window).height())});
		...
		render: function(){
			...
			return (
				React.createElement("div", {className: "buttonpanel"}, 
					React.createElement(ButtonList, {colours: colours, buttons: this.state.buttons, buttonwidth: this.state.buttonwidth, windowheight: this.state.windowheight})
				)
			)
		}
	});
	...
	var ButtonList = React.createClass({displayName: "ButtonList",
		...
		render: function(){
			...
			var columns = this.props.buttons.map(function(item, i){
				...
				return (
					 React.createElement(ButtonColumn, {buttoncount: buttoncount-item.buttons.length, colours: this.props.colours, clickhandler: this.handleModalClick, column: i, left: left, buttonwidth: this.props.buttonwidth, buttonheight: this.props.windowheight / item.buttons.length, category: item.category, buttons: item.buttons})
				);
			...
			return(	
				React.createElement("div", null, 
					...
					React.createElement(modal.Modal, {ref: "modal", style: modalStyle, show: false, header: this.state.currentbutton.name, buttons: buttons}, 
						React.createElement("p", null, this.state.currentbutton.name)
					)
				)
			)
