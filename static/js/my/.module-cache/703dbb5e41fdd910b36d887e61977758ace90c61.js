	...
	var MainPanel = React.createClass({displayName: "MainPanel",
		...
		getButtonsFromServer: function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					this.setState({buttons:data, buttonwidth:Math.floor($(window).width()/data.length), buttonheight:Math.floor($(window).height()/maxbuttons)});
		...
		componentDidMount: function(){
			...
			$(window).resize(function(){
				this.setState({buttonwidth:Math.floor($(window).width()/data.length), buttonheight:Math.floor($(window).height()/maxbuttons)});
			});
		},
